...
class ads_purchase_order(ads_data):
    ...
    def process(self, pool, cr, picking):
        """
        Triggers _process_po_picking to handle the processing of a PO picking.
        ...
        self._process_po_picking(pool, cr, picking_name, [line for line in picking_lines if line['move_type'] == 'in'])
        self._process_so_picking(pool, cr, picking_name, [line for line in picking_lines if line['move_type'] == 'out'])
    ...
    def _process_po_picking(self, pool, cr, picking_name, picking_lines):
        """
    ...
    def _process_so_picking(self, pool, cr, picking_name, picking_lines):
        """
        Executes the delivery wizard for an OUT based on self.data received from ADS
        @param pool: OpenERP object pool
        @param cursor cr: OpenERP database cursor
        @param str picking_name: Name of the picking to be processed
        @param list picking_lines: A list of picking move data. Refer to self._extract_to_process
        ...
        for move in wizard.move_ids:
            if move.product_id.x_new_ref in [line['name'] for line in picking_lines]:
                new_quantity = sum([line['quantity'] for line in picking_lines if line['name'] == move.product_id.x_new_ref])
                pool.get('stock.partial.picking.line').write(cr, 1, move.id, {'quantity': new_quantity})
            else:
                pool.get('stock.partial.picking.line').write(cr, 1, move.id, {'quantity': 0})
